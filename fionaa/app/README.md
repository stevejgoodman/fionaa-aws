...
From `fionaa/app`, run `uv sync --group dev --python 3.13`, then
`uv run pytest tests`. Build the distribution with `uv build --wheel`.
The wheel includes the policy Markdown alongside the code.

`main.py` is the AgentCore source-archive bootstrap; the application entrypoint
lives in `src/fionaa/main.py`. AgentCore's packager copies source and dependencies
but does not install the project wheel, so the bootstrap adds its adjacent `src/`
directory via `site.addsitedir`. Installed-package callers need no path setup.
The configured code location is `app/`; keep `src/` with the bootstrap in archives.

Node evaluations run from `fionaa/agentcore` using
`../app/.venv/bin/deepeval test run ../evals/node/test_policy_check.py`.
Calibration runs from `fionaa/` using
