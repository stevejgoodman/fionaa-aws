...
with patch.object(boto3, "client", side_effect=AssertionError("client during import")):
    from fionaa import schemas
    assert "fionaa.storage" not in __import__("sys").modules
    assert "fionaa.workflow.state" not in __import__("sys").modules
    from fionaa.domain.applications import ApplicationFormSchema
    from fionaa.domain.documents import BankStatementSchema
    ...
    from fionaa.model import load
    with patch.object(load, "load_model", side_effect=AssertionError("model during import")):
        from fionaa import graph
        graph.build_graph()
'''
